...
func assign(lhs reflect.Value, rhs ...string) error {
	...
	case reflect.Slice:
		...
		for _, rh := range rhs {
			elem, isPtr := newValue(lhs.Type().Elem())
			err := assign(elem, rh)
			if err != nil {
				return err
			}
	...
	case reflect.Map:
		...
		key, isKeyPtr := newValue(lhs.Type().Key())
		value, isValuePtr := newValue(lhs.Type().Elem())
...
// newValue allocates a new value of the given reflect.Type and returns it as a
// reflect.Value. If the provided type is a pointer type, newValue returns a
...
func newValue(typ reflect.Type) (reflect.Value, bool) {
	isPtr := isPointer(typ)
	if isPtr {
		typ = typ.Elem()
	}
